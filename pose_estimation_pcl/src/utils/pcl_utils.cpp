...
void filter_floor_clouds(
    ...
    pcl::transformPointCloud(*cloud, *cloud_in_map_frame, camera_to_map_transform);
    ...
    pass.setFilterLimits(height_threshold, std::numeric_limits<float>::max());
     ...
     pcl::transformPointCloud(*filtered_in_map, *cloud, camera_to_map_transform.inverse());
...
PlaneSegmentationResult detect_and_remove_planes(
    ...
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr floor_cloud(new pcl::PointCloud<pcl::PointXYZRGB>(*working_cloud));
    ...
    while (plane_count < max_planes && 
        ...
        is_floor = check_floor_plane(plane_cloud, coefficients, camera_to_map_transform, height_threshold, logger);
        // check if the segment is represent the floor
        if (is_floor) {
            // safe plane coefficient
