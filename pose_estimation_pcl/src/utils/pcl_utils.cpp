...
namespace pcl_utils
{
        ...
        const rclcpp::Logger &logger)
    {
        ...
        if (pcl::io::loadPCDFile<pcl::PointXYZRGB>(model_path, *model_cloud) == -1)
        {
            RCLCPP_ERROR(logger, "Failed to load model PCD file: %s", model_path.c_str());
        }
        else
        {
        ...
        assert (model_cloud->empty() == false);
